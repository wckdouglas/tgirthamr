...
{
	vector <int> baseCount(4);
	baseCount[0] = A;
	baseCount[1] = C;
	baseCount[2] = T;
	baseCount[3] = G;
	int ref = cov - A - T - C - G;
	int maxMismatch = *max_element(baseCount.begin(),baseCount.end());
	int hetMismatch =  cov - maxMismatch - ref + deletion;
	return hetMismatch;
}
...
DataFrame transformDF (DataFrame df, double seqErr, double pCutOff, Function binom)
{
	...
	for (int i = 0; i < A.size(); i++)
	{
		...
		mergedAbbrev[i] = mergeType(abbrev[i]);
	}
